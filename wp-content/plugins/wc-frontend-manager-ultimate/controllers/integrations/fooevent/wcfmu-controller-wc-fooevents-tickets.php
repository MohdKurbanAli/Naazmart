...
class WCFMu_Event_Tickets_Controller {
	...
	public function processing() {
		...
		// Load FooEvents_Barcode_Helper compatible version
		if ( isset( $config->class_path ) && file_exists( $config->class_path . 'class-fooevents-barcode-helper.php' ) ) {
			require_once( $config->class_path . 'class-fooevents-barcode-helper.php' );
		} else {
		...
		$wcfm_event_tickets_json = '{
			...
			if ( !empty( $wcfm_event_tickets_array ) ) {
				...
				foreach ( $wcfm_event_tickets_array as $wcfm_event_tickets_single ) {
					...
					// Barcode Option
					$barcodeURL =  isset( $config->barcodeURL ) ? $config->barcodeURL : $config->barcode_url;
					$barcodePath =  isset( $config->barcodePath ) ? $config->barcodePath : $config->barcode_path;
					$eventPluginURL = isset( $config->eventPluginURL ) ? $config->eventPluginURL : $config->event_plugin_url;
					$pdfTicketURL = isset( $config->pdfTicketURL ) ? $config->pdfTicketURL : $config->pdf_ticket_url;
					$pdfTicketPath = isset( $config->pdfTicketPath ) ? $config->pdfTicketPath : $config->pdf_ticket_path;

					if (!file_exists($barcodePath.$WooCommerceEventsTicketID.'.png')) {
					...
					if ( WCFMu_Dependencies::wcfm_wc_fooevents_pdfticket() ) {
						...
						$urlPath = $pdfTicketURL.$fileName.'.pdf';
						$filePath = $pdfTicketPath.$fileName.'.pdf';
						...
						if( !file_exists( $filePath ) ) {
							...
							$FooEvents_PDF_Tickets->generate_ticket( $productID, $ticket, $barcodePath, $eventPluginURL );
						}
